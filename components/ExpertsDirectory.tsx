...
/* Derive partner list from data so it stays in sync */
const ALL_PARTNERS = [...new Set(experts.map((e) => e.national_partner))].sort();

/* Derive province codes that appear in data */
const ALL_PROVINCE_CODES = [
  ...new Set(experts.flatMap((e) => e.provinces_covered)),
].sort();
...
function SpecialtyPill({ label }: { label: string }) {
  ...
  return (
    <span
      className={`inline-block text-xs font-medium px-2.5 py-1 rounded-full border ${style} whitespace-nowrap`}
    >
      {label}
    </span>
  );
          ...
          <div className="flex items-start gap-4">
            {expert.logo_url ? (
              ...
              <img
                ...
                className="w-14 h-14 object-contain rounded-xl border border-plum/8"
              />
            ) : (
            ...
            <div className="flex flex-col gap-0.5 min-w-0">
              ...
              <span className="text-xs text-magenta/70 font-medium mt-0.5">{expert.national_partner}</span>
            </div>
          </div>
...
function TogglePill({
  ...
  return (
    ...
    >
      <span
        className={`w-2 h-2 rounded-full transition-colors ${active ? "bg-peach-light" : "bg-plum/20"}`}
      />
      {label}
    </button>
  );
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-10 md:mb-14">
          ...
          <p className="text-plum/50 text-base max-w-lg mx-auto leading-7">
            Vetted intellectual property professionals connected through the ElevateIP national network. Find the right expert for your stage and region.
          </p>
        </div>
